...
ALLERGEN_PAREN_RE = re.compile(r'\s*\(알레르기 유발 성분:\s*([^)]*)\)')
...
def startup():
    ...
    # 기존 DB에 allergens 컬럼이 없으면 추가 (신규 컬럼은 create_all이 채워주지 않음)
    with engine.connect() as conn:
        has_column = conn.execute(text(
            "SELECT 1 FROM information_schema.columns "
            "WHERE table_name = 'menus' AND column_name = 'allergens'"
        )).first()
        if not has_column:
            conn.execute(text("ALTER TABLE menus ADD COLUMN allergens VARCHAR(200) DEFAULT ''"))
            conn.commit()
    ...
    try:
        ...
        if db.query(Menu).count() == 0:
            ...
            allergen_map = {m.name: m.allergens for m in MENU_SEED}
            for menu in db.query(Menu).all():
                if not menu.description and desc_map.get(menu.name):
                    menu.description = desc_map[menu.name]
                if not menu.allergens:
                    # 예전 방식(설명 문구 안에 "(알레르기 유발 성분: ...)")으로 저장된 값이 있으면 분리
                    match = ALLERGEN_PAREN_RE.search(menu.description or "")
                    if match:
                        extracted = match.group(1).strip()
                        menu.allergens   = "" if extracted == "없음" else extracted
                        menu.description = ALLERGEN_PAREN_RE.sub("", menu.description).strip()
                    elif allergen_map.get(menu.name):
                        menu.allergens = allergen_map[menu.name]
        ...
        FAQ_SEED = [
            ...
            FAQ(question="알레르기 유발 성분 알 수 있나요?", answer="메뉴별 알레르기 유발 성분을 안내하고 있어요. 우유, 계란, 밀, 대두, 견과류 등이 포함된 메뉴는 성분 정보에 표시돼요.", keywords=["알레르기","성분"]),
